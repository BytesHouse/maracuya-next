...
const FormCart = () => {
  ...
  useEffect(() => {
    setDelivery(
      cart.length <= 3 ? 55 : cart.length > 3 && cart.length <= 6 ? 45 : 35
    );
  }, [cart]);
  ...
  return (
    <div id="contacts" className={styles.wrapper}>
      ...
      <form ref={form} onSubmit={handleSubmit}>
        <input id="id" name="id" type="text" hidden value={formData.id} />
        <input
          id="message"
          name="message"
          type="text"
            ...
            <div className={styles.prodList}>
                ...
                {cart.map((item, index) => (
                  <li key={index} className={styles.prodItem}>
                    ...
                    <p className={styles.price}>
                      {Number(item.price * item.count).toFixed(2)} <Ruble />
                    </p>
                  </li>
                ))}
              </ul>
              ...
              <div className={styles.total}>
                ...
                <div>
                  <p>Итого:</p>
                  <p className={styles.totalSum}>
                    {(total + delivery).toFixed(2)} <Ruble />
                  </p>
                </div>
              </div>
            </div>
